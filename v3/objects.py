...
class Game:
    ...
    def perform_move(self, piece_x, piece_y, target_x, target_y, is_capture, special_move):
        ...
        if isinstance(piece, Rook):
            if piece.colour == WHITE:
                if self.white_castling_rights[0] and piece_x == 0 and piece_y == self.height-1:
                    self.white_castling_rights[0] = False
                if self.white_castling_rights[1] and piece_x == self.width-1 and piece_y == self.height-1:
                    self.white_castling_rights[1] = False
            else:
                if self.black_castling_rights[1] and piece_x == 0 and piece_y == 0:
                    self.black_castling_rights[1] = False
                if self.black_castling_rights[0] and piece_x == self.width-1 and piece_y == 0:
                    self.black_castling_rights[0] = False

        if isinstance(target, Rook):
            if piece.colour == WHITE:
                if self.white_castling_rights[0] and target_x == 0 and target_y == self.height-1:
                    self.white_castling_rights[0] = False
                if self.white_castling_rights[1] and target_x == self.width-1 and target_y == self.height-1:
                    self.white_castling_rights[1] = False
            else:
                if self.black_castling_rights[1] and target_x == 0 and target_y == 0:
                    self.black_castling_rights[1] = False
                if self.black_castling_rights[0] and target_x == self.width-1 and target_y == 0:
                    self.black_castling_rights[0] = False
        ...
        if special_move is None:
            ...
            match special_move:
                ...
                case 'castle_left':
                    if piece.colour == WHITE:
                        rook_x = 0
                        rook_y = self.height-1
                        rook = self.get_piece(rook_x, rook_y)
                        self.set_piece(rook_x, rook_y, None)
                        self.set_piece(target_x+1,target_y,rook)
                        rook.set_position(target_x+1,target_y)
                    else:
                        rook_x = self.width-1
                        rook_y = 0
                        rook = self.get_piece(rook_x, rook_y)
    ...
    def is_valid_move(self, piece_x, piece_y, target_x, target_y, is_capture, special_move):
        ...
        elif special_move == 'castle_left':
            if self.in_check:
                valid = False
            else:
                check_x = target_x - piece.colour
                self.set_piece(piece_x, piece_y, None)
                self.set_piece(check_x, target_y, piece)
                piece.set_position(check_x, target_y)
                if self.is_in_check(piece.colour):
                    valid = False
                self.set_piece(piece_x, piece_y, piece)
                self.set_piece(check_x, target_y, None)
                piece.set_position(piece_x, piece_y)

        elif special_move == 'castle_right':
            if self.in_check:
                valid = False
            else:
                check_x = target_x + piece.colour
...
class King(Piece):
    ...
    def get_possible_moves(self, game):
        ...
        possible_moves = game.get_set_moves(moves, self.x, self.y, self.colour)

        direction = self.colour
        new_y = self.y
        rights = game.white_castling_rights if self.colour == WHITE else game.black_castling_rights

        new_x = self.x + direction
        if rights[0] == True and game.inside_board(new_x, new_y) and game.get_piece_colour(new_x, new_y) == None: #left
            new_x = self.x + direction * 2
            if game.inside_board(new_x, new_y) and game.get_piece_colour(new_x, new_y) == None:
                possible_moves.append((new_x, new_y, False, 'castle_left'))
                #print(f'{self.colour} castle left')

        new_x = self.x + (-direction)
        if rights[1] == True and game.inside_board(new_x, new_y) and game.get_piece_colour(new_x, new_y) == None: #right
            new_x = self.x + (-direction) * 2
            if game.inside_board(new_x, new_y) and game.get_piece_colour(new_x, new_y) == None:
                possible_moves.append((new_x, new_y, False, 'castle_right'))
                #print(f'{self.colour}castle right')
